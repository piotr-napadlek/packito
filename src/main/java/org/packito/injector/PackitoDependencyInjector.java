...
public class PackitoDependencyInjector {
    ...
    public void injectDependenciesDeep(ClassToTest classToTest, List<Class<?>> classesInAPackage, DependenciesContainer dependencies) {
        try {
            ...
            throw new PackitoInitializationException(e.toString());
        }
    ...
    private void injectDependencies(Object instance, List<Class<?>> classesInAPackage, DependenciesContainer dependencies) {
        ...
        for (Field field : fieldsToInject) {
            ...
            try {
                ...
                e.printStackTrace();
                throw new PackitoInitializationException("Could not inject field " + field.getName() + " of type " + field.getType()
                                                                 + " in class " + klass.getName());
            } catch (InstantiationException e) {
                e.printStackTrace();
                throw new PackitoInitializationException("Could not instantiate class for a field " + field.getName() + " of type " + field.getType()
                                                                 + " in class " + klass.getName());
            }
        }
    }
    ...
    private Optional<Class<?>> getAssignableClass(Class<?> targetClass, Collection<Class<?>> potentialClasses) {
        ...
        if (matchedClasses.size() > 1) {
            throw new PackitoInitializationException("There is ambiguity for injection of field of type " + targetClass.getName()
